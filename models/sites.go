...
var SiteWhere = struct {
	...
	URL:  whereHelperstring{field: "`sites`.`url`"},
}
...
var (
	siteAllColumns            = []string{"id", "name", "url"}
	siteColumnsWithoutDefault = []string{"name", "url"}
...
func (o *Site) Articles(mods ...qm.QueryMod) articleQuery {
...
func (siteL) LoadArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSite interface{}, mods queries.Applicator) error {
	var slice []*Site
	var object *Site
	...
	query := NewQuery(qm.From(`articles`), qm.WhereIn(`articles.sites_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Site) AddArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	var err error
