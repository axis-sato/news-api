...
type Tag struct {
	...
	IsFollowed int8   `boil:"is_followed" json:"is_followed" toml:"is_followed" yaml:"is_followed"`
...
var TagWhere = struct {
	...
	IsFollowed whereHelperint8
}{
	ID:         whereHelperuint{field: "`tags`.`id`"},
	Name:       whereHelperstring{field: "`tags`.`name`"},
	IsFollowed: whereHelperint8{field: "`tags`.`is_followed`"},
}
...
var (
	tagAllColumns            = []string{"id", "name", "is_followed"}
	tagColumnsWithoutDefault = []string{"name", "is_followed"}
...
func (o *Tag) Articles(mods ...qm.QueryMod) articleQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`article_tags` on `articles`.`id` = `article_tags`.`article_id`"),
		qm.Where("`article_tags`.`tag_id`=?", o.ID),
	)
...
func (tagL) LoadArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	var slice []*Tag
	var object *Tag
	...
	query := NewQuery(
		qm.Select("`articles`.*, `a`.`tag_id`"),
		qm.From("`articles`"),
		qm.InnerJoin("`article_tags` as `a` on `articles`.`id` = `a`.`article_id`"),
...
func (o *Tag) AddArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into `article_tags` (`tag_id`, `article_id`) values (?, ?)"
...
// Sets o.R.Tags's Articles accordingly.
// Replaces o.R.Articles with related.
// Sets related.R.Tags's Articles accordingly.
func (o *Tag) SetArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	query := "delete from `article_tags` where `tag_id` = ?"
	...
	removeArticlesFromTagsSlice(o, related)
	if o.R != nil {
		o.R.Articles = nil
	}
...
// Sets related.R.Tags.
func (o *Tag) RemoveArticles(ctx context.Context, exec boil.ContextExecutor, related ...*Article) error {
	var err error
	query := fmt.Sprintf(
		"delete from `article_tags` where `tag_id` = ? and `article_id` in (%s)",
...
func removeArticlesFromTagsSlice(o *Tag, related []*Article) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
